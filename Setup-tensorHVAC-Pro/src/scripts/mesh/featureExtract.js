...
function listIndexedFeatureEntries(text, prefix) {
  const rx = new RegExp(String.raw`\\b${prefix}_(\\d+)\\.stl\\s*\\{`, "g");
...
`;
  ...
  if (!flags.object) {
    ...
    sfeText = removeFeatureEntry(sfeText, `object`); // legacy non-indexed
    // Safety purge
    sfeText = sfeText.replace(/(^|\n)\s*object_\d+\.stl\s*\{[\s\S]*?\}\s*/g, "\n");
  } else {
